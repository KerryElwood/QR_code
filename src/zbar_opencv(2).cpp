...
int main(int argc, char **argv) {
    ...
    image_converter.K << 568.996140852	,0	,643.21055941,
        ...
        if (symbol->get_location_size() == 4) 
        {
            ...
            switch(image_converter.state)
            {
                ...
                case 2:
                    ...
                    cout<< "target location: "<< image_converter.P3DN(0)<<","<<image_converter.P3DN(1)<<","<<image_converter.P3DN(2)<<endl;
                    ...
                    markern.scale.x = 0.17;
                    markern.scale.y = 0.17;
...
Vector3d ImageConverter::simtriangle()
{
    ...
    // image_converter.campnt(0) = (pic0pnts(0)-320) * image_converter.L/image_converter.length;//像素坐标*实际宽度/像素宽度
    // image_converter.campnt(1) = (pic0pnts(1)-240) * image_converter.L/image_converter.length;   
    image_converter.campnt(2) = (image_converter.campnt(0)-320)*image_converter.fx/pic0pnts(0);// x_real*fx/像素坐标

    out = (campnt - C1)*image_converter.R1.inverse();
